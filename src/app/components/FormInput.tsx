...
const FormInput: React.FC<FormInputProps> = ({
  ...
  if (type === 'text' || type === 'password' || type === 'email') {
    ...
    return (
      <label
        htmlFor={id}
        className="rounded border-[1px] border-input-gray px-[1rem] py-[0.625rem] text-[1.125rem] hover:bg-gray-200 cursor-pointer "
      >
        {label}
        <input
          type={type}
    ...
    return (
      <label htmlFor={id} key={id} className="flex flex-col gap-2 w-full">
        ...
        <textarea
          ...
          required={required}
          className="text-white  input-shadow p-[0.625rem] border border-input-gray bg-medium-gray rounded-md w-full"
        />
        {error && <p className="text-coral">{error}</p>}
      </label>
    );
